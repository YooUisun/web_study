...
//		return "redirect:/re/re2";	//그 경로로 다시 요청 ! 요청 할 주소 
//		return "redirect:/re/re2?msg=re4 redirect msg"; //경로에 파라미터 추가
	...
	// forward는 거의 안 씀 redirect 를 많이 씀.
	@GetMapping("/re/re5")
	...
	public String listTest(@RequestParam String type) {
		...
		if (type.equals("str")) {
			 ...
			 * 
			 * for(int i=1; i<=5; i++) { memberList.add(new Member("아이디"+i, "비번"+i,
			 * "이름"+i)); } model.addAttribute("memberList", memberList);
			 * 
			 * return "jstl/member";
			 */
		}
