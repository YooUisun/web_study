...
public class CookieController {
	...
	public String readCookie(HttpServletRequest request) {
		...
		/*
		 * for(Cookie ck : cookies) { System.out.println(ck.getName() + " " +
		 * ck.getValue() );
		 * 
		 * if(ck.getName().equals("weather")) { request.setAttribute("weather",
		 * ck.getValue()); } }
		 */
	...
	public String idCookieAction(HttpServletRequest request, HttpServletResponse response) {
		...
		// id pw 비교 <----> DB 저장된 데이터
		// > 로그인 성공 -> 보통 마이페이지/메인페이지/기존페이지로 이동을 해야함.
//		return "redirect:/read-cookie2"; // 리다이렉트 할 땐 주소를 넣어야함. 리턴값을 넣는게 아니라
		...
		// > 로그인 실패 -> 다시 로그인 하는 화면을 보여주고
		// > 다시 로그인 하는 화면을 보여주는 화면
		return "cookie/id-cookie";
	}
}
