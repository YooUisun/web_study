...
public class Rest2Controller {
	...
	public String rest6() {
		...
		/*
		 * 단순 텍스트 abc,qwe,name1,CUS return "abc,qwe,name1,CUS";
		 * 
		 * 많은 데이터 전달 -> 표현 규칙 -> JSON
		 * 
		 * { "id":"abc", "pw":"qwe", "name":"name1", "userType":"CUS" }
		 */
		/*
		 * JSON 포맷으로 return 을 하려면? 1) 직접 포맷대로 수동으로 작성
		 * 
		 * 
		 * return "{" + "	\"id\":\"abc\"," + "	\"pw\":\"qwe\"," +
		...
		// jackson라이브러리 활용
		// 객체 -> json 포맷 String 변환 -> 리턴
		return jsonStr;
	}

	@GetMapping("/rest/rest7") // 시전 조건 : jackson 라이브러리 존재
	...
	public ApiDelivery rest10() {
		...
		apiDelivery.setPhone("010-1234-1233");

		ApiStore apiStore = new ApiStore();
		apiStore.setName("롯데리아");
		apiStore.setAddress("대흥동 399-5번지 1층");
...
//		apiResponseHeader.setResultCode("정상");
		apiResponseHeader.setResultCode(ApiCommonCode.API_RESULT_SUCCESS);
		apiResponseHeader.setResultMessage(ApiCommonCode.API_RESULT_SUCCESS);
